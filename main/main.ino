...
bool air(int number){
  ...
  if       (analogRead(Air_Pin[number]) < Air_Decide[number])   //与阈值对比
  {
  ...
  }else if (analogRead(Air_Pin[number]) > Air_Decide[number])
  {
...
void SelfCheck(){
  ...
  int readings[5];
  int average = 0;
  for(int i = 0;i < 6;i++){           //取值5次并计算平均
    for(int j = 0;j < 5; j++){
      readings[j] = analogRead(Air_Pin[i]);
      delay(100);
    };
    for(int j = 0; j < 5; j++){
      average += readings[j];
...
void SelfCheck_Fail(int number,int var){
  char k = (char)(number + '0');   //转换int为char（keybord.press/release只能char）
  char l = (char)(var + '0');   //转换int为char（keybord.press/release只能char）
  u8g2.clearBuffer();
  // 在左上角显示 "SelfCheck False"
  u8g2.drawStr(0, 8, "SelfCheck False");
  // 在下面显示 "Info"
  u8g2.drawStr(0, 16, "Pin:");
  u8g2.drawStr(24, 24, k);
  u8g2.drawStr(50, 24, l);
  u8g2.drawStr(0, 32, "again:5s");
